...
SMODS.DrawStep {
    ...
    order = 40,
    func = function(self, layer)
    if self.config.center.key == 'j_poke_unown_swarm' then return end
    ...
    func = function(self)
        if self.config.center.soul_pos and (self.config.center.discovered or self.bypass_discovery_center) then
            ...
            if self.edition then 
                ...
                if (edition.apply_to_float or self.config.center.poke_apply_soul_edition) and self.children.floating_sprite then
