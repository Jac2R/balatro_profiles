...
function Card:set_cost_value()
    ...
    if self.ability.set == 'Joker' or self.ability.consumeable then self.cost = math.max(0, self.cost - 1 * #SMODS.find_card('j_poke_minun')) end
...
function Card:open()
    if self.ability.set == "Booster" then
        ...
        if self.ability.name:find('Celestial') and next(SMODS.find_card("j_poke_xatu")) then
            self.ability.extra = #pokermon.get_available_planet_cards()
